...
def turn_left(speed,angle):
    motor_left.run_to_rel_pos(position_sp=-(154*angle)/90, speed_sp=speed)
    motor_right.run_to_rel_pos(position_sp=(154*angle)/90, speed_sp=speed)
...
def move_distance_in_mm(distance_mm, speed):
    dist = (distance_mm/1000) * (360/0.176)
    #print(dist)
    motor_left.run_to_rel_pos(position_sp = dist, speed_sp=speed, stop_action = "hold")
    motor_right.run_to_rel_pos(position_sp = dist, speed_sp=speed, stop_action = "hold")
    motor_left.wait_while('running')
    motor_right.wait_while('running')
    motor_left.stop()
    motor_right.stop()
...
def drive_to_home_zone(hypotenuse, current_angle, add):
    ...
    gy.mode = 'GYRO-ANG'
    sleep(1)
    move_distance_in_mm(abs(distance + add),400)
    gripper("open")
    harvested += 1  #keeping count of the harvested fruits
    move_distance_in_mm(-add, 500)
    turn_left(200,70)
    end = time.time()
...
def drive_from_acre_to_line(hypotenuse, current_angle):
    ...
    gy.mode = 'GYRO-ANG'
    sleep(1)
    move_distance_in_mm(-abs(distance),400)
...
#**********************************************
def TurnLeft2():
    motor_left.run_to_rel_pos(position_sp=-154, speed_sp=100)#154
    motor_right.run_to_rel_pos(position_sp=154, speed_sp=100)
    motor_left.wait_while('running')
    motor_right.wait_while('running')
...
def scanning():
    ...
    while(dist > search_space):
        while(dist > search_space and gy.value()>-83): # its suppose to be -90 but i put -75 because gyro is not accurate
            #print(dist)
        ...
        while(dist > search_space and gy.value()<-3):
            #print(dist)
            motor_left.run_forever(speed_sp=scanning_speed, stop_action = 'hold')
            motor_right.run_forever(speed_sp=-scanning_speed, stop_action = 'hold')
            dist = us.value()
    ...
    if(dist > 500):
        move_distance_in_mm(dist/2,500)
        while(us.value() < search_space):# and gy.value()>-90):
            motor_left.run_forever(speed_sp=-50, stop_action = 'hold')
            motor_right.run_forever(speed_sp=50, stop_action = 'hold')    
        motor_left.stop()
        ...
        motor_left.stop()
        motor_right.stop()
        move_distance_in_mm(dist/2+10,400)
    else:
        move_distance_in_mm(dist+20,400)
    #approaching
    ...
    if (cl.getCalibratedColorString() == fruit):
        ...
        drive_to_home_zone(dist, abs(gy.value()), 400)
...
while (harvested<1 and current_time<600): #5 minutes = 300s
    scanning()

move_distance_in_mm(-total_distance,400)  
