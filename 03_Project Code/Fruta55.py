...
def drive_to_home_zone(hypotenuse, current_angle, add):
    ...
    turn_right(100,current_angle + 90)
    gy.mode = 'GYRO-RATE'
    gy.mode = 'GYRO-ANG'
    sleep(1)
...
def scanning1():
    ...
    while(dist > search_space):
        while(dist > search_space and gy.value()>-82): # its suppose to be -90 but i put -75 because gyro is not accurate
            #print(dist)
...
while (harvested<1 and current_time<120):
    beta = scanning1
    print(beta)
    if (beta != 0):
        scanning2()
