...
#**********************************************
def TurnLeft2():
    motor_left.run_to_rel_pos(position_sp=-77, speed_sp=100)#154
    motor_right.run_to_rel_pos(position_sp=77, speed_sp=100)
    motor_left.wait_while('running')
    motor_right.wait_while('running')
...
def scanning(degrees1, degrees2):
    global total_distance, thetha
    gy.mode = 'GYRO-RATE'
    gy.mode = 'GYRO-ANG'
    sleep(1)
    search_space = 1300
    scanning_speed = 50
    dist = us.value()
    while(dist > search_space or thetha == None): #On the second scanning the distance is smaller than the search space and thetha is not None, so it doesn't scan
        while(dist > search_space and gy.value()>degrees1): # its suppose to be -90 but i put -75 because gyro is not accurate
            print(gy.value(), degrees1)
        ...
        while(dist > search_space and gy.value()<degrees2):
            print(gy.value(), degrees2)
            motor_left.run_forever(speed_sp=scanning_speed, stop_action = 'hold')
            motor_right.run_forever(speed_sp=-scanning_speed, stop_action = 'hold')
            dist = us.value()
        motor_left.stop()
