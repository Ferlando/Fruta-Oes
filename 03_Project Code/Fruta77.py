...
def drive_from_acre_to_line(hypotenuse, current_angle):
    ...
    print(distance)
    turn_left(1000, 80)
    turn_right(200, current_angle+10)
    total_distance += abs(hypotenuse*(math.cos(math.radians(current_angle))))
...
def scanning():
    ...
    scanning_speed = 50
    dist = us.value()
    while(dist > search_space or thetha == None): #or gy.value()<-30): #While the distance is greater than the search space and thetha is None:
        while(dist > search_space and gy.value()>-80): #Here we turn left while the angle is bigger than -80
            #print(dist)
        ...
        while(dist > search_space and gy.value()<-5):
            #print(dist)
            motor_left.run_forever(speed_sp=scanning_speed, stop_action = 'hold')
            motor_right.run_forever(speed_sp=-scanning_speed, stop_action = 'hold')
            #if (gy.value() < -30):
        ...
        thetha = gy.value() #Angle value saved
        #if (gy.value() < -30):
        #    dist = us.value()

        if(dist > search_space): #If the distance is still greater than the seearch space:
            move_distance_in_mm(200) #It moves a little to thee front
            total_distance += 200
            
        end = time.time()
        current_time += (end - start)
        
    while(us.value() < search_space):# If the distance to the object is smaller than the search space, it turns to the left
    ...
    alpha = gy.value() - thetha #The angle with which the object was detected again minus thetha becomes alpha
...
while (harvested<2 and current_time<4500):
    scanning()
    print(total_distance, "Total distance")
    if(total_distance >= 3700):
